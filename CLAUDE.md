...
- **Placement memory** (`preservelayout`): `manage()` → `place()` looks the window's WM_CLASS class up in `layoutfile` (`app:workspace:column:percent` lines) and tiles it on that workspace to the right of its selected column (last if none), switching to that workspace unless adopting at startup; `savelayout()` leaves the column field empty (for now), a hand-written index only counts when the window lands on a workspace other than the current one, and a hand-written `-1` index (`IDXFLOAT`) opens the app floating (`manage()` does the rule lookup so it can decide that before placing; `Rule.idx` is `IDXNONE` when the field is empty); every command that moves/reorders/resizes a tiled window ends with `savelayout()`, which rewrites the file atomically (temp + rename). The file is re-read on every lookup and save, so there is no in-memory copy and hand edits apply immediately. An `app:::[percent]` line (ws = -1 after parsing) opts the app out: it opens at the current selection and `savelayout()` leaves the line alone.
...
- **Control socket**: `commands[]` in `hwm.c` names every bindable command with its argument type (`ANONE`/`AINT`/`AFLOAT`/`AARGV`, plus `ADUMP`/`AKEYS` for the two queries) and is the whole command surface. hwm listens on `$XDG_RUNTIME_DIR/hwm$DISPLAY.sock` (`initsock()`, an fd in `run()`'s `select()`); `servecmd()` reads one client's NUL-separated argv until EOF, `runcmd()` validates, calls `syncactivemon()` (so it acts where the pointer is, like a keypress) and the command, and replies `ok\n` + payload or `err <why>\n`, then closes. `hwm send`/`hwm dump`/`hwm keys` is the same binary in client mode (`sendcmd()`), which validates the command name locally before connecting. `keys` (`dumpkeys()`) prints the running config's bindings — modifiers+key, the command as `hwm send` spells it, and a description: `Key.desc` from config.h (every base binding has one; the compiler's `-Wmissing-field-initializers` insists), else the command's help — tab-separated, argv words shell-quoted, so hmenu's `keys` mode can run a row with `sh -c "hwm send $cmd"`. Accepted fds are `FD_CLOEXEC` so `spawn`'s children don't hold the client open, and get 1 s socket timeouts so a stuck client can't stall the WM. The layout command that sends a window to a workspace is `sendws`, not `sendto`: that name is libc's.
